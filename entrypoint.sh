...
sudo sed -i 's/-fcf-protection/-fcf-protection -fuse-ld=lld -ftree-vectorize/g' /etc/makepkg.conf;
sudo sed -i 's/#RUSTFLAGS="-C opt-level=2"/RUSTFLAGS="-C opt-level=2 -C strip=symbols -C link-arg=-fuse-ld=lld -C target-cpu='$march'"/g' /etc/makepkg.conf;
